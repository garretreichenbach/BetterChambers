...
public class EffectAddOn extends RecharchableActivatableDurationSingleModule{
		...
		switch (getPowerConsumptionEffect(type)) {
			case THRUSTER_BLAST_POWER_CONSUMPTION_CHARGING:
				if(man instanceof ShipManagerContainer){
					...
					return getConfigManager().apply(getPowerConsumptionEffect(type), ((ShipManagerContainer) man).getThrusterElementManager().getPowerConsumedPerSecondCharging() * 0.35);
				}
			default:
	...
	public StatusEffectType getPowerConsumptionEffect(BlockEffectTypes type){
		switch(type){
			case TAKE_OFF: return StatusEffectType.THRUSTER_BLAST_POWER_CONSUMPTION_CHARGING;
			default: return StatusEffectType.THRUSTER_BLAST_POWER_CONSUMPTION_CHARGING;//throw new RuntimeException("no mapped consumption: "+type.getName());
		}
	}
	private int getMaxCharges(BlockEffectTypes type) {
		switch(type){
			case TAKE_OFF: return getSegmentController().getConfigManager().apply(StatusEffectType.THRUSTER_BLAST_MULTI_CHARGE_COUNT, 1);
			default: return 1;//throw new RuntimeException("no mapped consumption: "+type.getName());
		}
	}
	...
	public float getChargeRateFull() {
		switch(type){
			case TAKE_OFF: return getConfigManager().apply(StatusEffectType.THRUSTER_BLAST_COOLDOWN, 1f);
	...
	public short getWeaponRowIcon() {
		switch(type){
			case TAKE_OFF: return ElementKeyMap.THRUSTER_ID;
			default: return ElementKeyMap.EFFECT_OVERDRIVE_COMPUTER;
		}
	}
	@Override

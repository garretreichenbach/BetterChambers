...
import thederpgamer.betterchambers.utils.DataUtils;
import thederpgamer.betterchambers.utils.DateUtils;
...
public class LogManager {
	...
	private static void logMessage(MessageType messageType, String message) {
		if(!messageQueue.contains(message) || messageType.equals(MessageType.CRITICAL)) {
			...
			String prefix = "[" + DateUtils.getTimeFormatted() + "] [BetterChambers] " + messageType.prefix;
			try {
				...
				logWriter.flush();
				DebugFile.log(builder.toString(), BetterChambers.getInstance());
			} catch(IOException exception) {
				exception.printStackTrace();
			}
